...
## 14. A `List<Struct>` parameter cannot have its elements' fields read (1.4.2, blocks the AOT build)

**Severity: blocking.** `forgen build src/main.dtr` stops here:

```
Code generation failed: [E0944] cannot resolve the receiver class for field access
'.dlv_subscription_id' in function 'bus_count_by_subscription': refusing to guess
the field offset
```

`forgen check` reports `Verified 100% OK` on the same sources, and `cargo build`
of `crates/kernel-wasm` is unaffected. Only AOT code generation fails.

This is finding 1 seen from the other side. 1.4.2 no longer guesses an offset it
cannot resolve - which is the right change, because guessing is what produced the
silent miscompilation - but it now refuses in a case where the receiver class is
plainly written down. The kernel's field names are already globally unique
(`scripts/style_check.py` is clean, 18 files, 38 struct types), so this is not the
duplicate-name case finding 1 describes.

### The repro

Committed as `probes/list_param_field/main.dtr`, so the claim is reproducible
rather than quoted. Two fields, one loop, no ambiguity:
...
pub fn count_by_sub( plan: List<Delivery>, sub_id: Int) -> Int {
    ...
    mut i = 0
    while i < plan.len() {
        if plan[i].dlv_subscription_id == sub_id { n = n + 1 }
        i = i + 1
    }
    return n
}
...
fn main() {
    ...
    println(int_to_str(count_by_sub(plan, 1)))
}
```

`forgen check` passes. `forgen build` fails with E0944.

### What decides it: whether the list is a parameter

| shape | result |
|-------|--------|
| `plan[i].field` where `plan` is a **local** of `List<Delivery>` | builds |
| `d.field` where `d` is a **parameter** of type `Delivery` | builds |
| `plan[i].field` where `plan` is a **parameter** of `List<Delivery>` | **E0944** |

Measured on `x86_64-pc-windows-msvc`, forgen 1.4.2. The same failure appears with
`view plan: List<Delivery>` and without it.

### Nothing in the language gets around it

Every one of these was tried against the repro and every one still fails:

- `let d = plan[i]` then `d.field` — the element is not typed either way
- `let d: Delivery = plan[i]` — the annotation does not help
- `mut local = plan` at the top, then `local[i].field` — copying the list does not
  type it
- `for d in plan` instead of indexing
- passing `plan[i]` to an accessor `fn f( view d: Delivery) -> Int` — the argument
  is what cannot be typed, so the accessor is never reached
- indexing through a second function `fn element( plan: List<Delivery>, i: Int) -> Delivery`

So this cannot be worked around in Datara source, and there is no SEAM to write:
there is no alternative spelling that compiles. The build stays red until the
compiler types a list parameter's element.

### Why it went unnoticed until now

`scripts/build.sh` runs the lint first because the linter catches what the
compiler cannot (finding 1). Both of those still pass. Stage 3 is the first stage
that invokes code generation, so before 1.4.2 the same sources produced a binary -
one that may well have been reading the wrong offsets, which is exactly what
finding 1 says happens silently. The 1.4.2 diagnostic did not introduce a bug; it
stopped hiding one. That is progress, and it is also a red build.

### Fix, in order

Make AOT code generation resolve the element class of a `List<T>` parameter - the
declared type is present in the AST, so this is a resolution gap rather than
missing information. Then add the repro above to the compiler's test suite: a
function parameter is the case the current tests miss, and a local is not a
substitute for it.
